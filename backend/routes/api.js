...
const workoutScheduleRouter = require('./workoutSchedule');

// 라우터 등록
router.use(authRouter); // /register, /login, /logout
router.use(userRouter); // /users
router.use(mealRouter); // /meals/*
router.use(workoutRouter); // /record
router.use(reportRouter); // /workouts/*
router.use(trainerRouter); // /trainer/members
router.use(scheduleRouter); // /trainer/schedule/*, /trainer/schedule/:trainerId
router.use(bookingRouter); // /trainer/schedule/book, /member/bookings
router.use(workoutScheduleRouter); // /workout-schedule/*

// 기본 응답 테스트
router.get('/', (req, res) => {
    res.send('API 라우터 정상 연결됨!');
});
