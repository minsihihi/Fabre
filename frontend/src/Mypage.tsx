...
function MypageMember() {
  ...
  const fetchProfileImage = async (userId: string) => {
    try {
      const response = await axios.get("https://13.209.19.146:3000/api/images/profile", {
        params: { userId },
      });
  ...
  const handleImageUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        const response = await axios.post("https://13.209.19.146:3000/api/upload/profile", formData, {
          headers: {
