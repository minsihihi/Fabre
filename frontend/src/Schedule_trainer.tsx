...
const TrainerScheduleGrid: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const scheduleRes = await axios.get('https://13.209.19.146:3000/api/trainer/schedule', {
        headers: { Authorization: `Bearer ${token}` },
      });
      setScheduleData(scheduleRes.data.schedules);

      const bookingsRes = await axios.get('https://13.209.19.146:3000/api/trainer/bookings', {
        headers: { Authorization: `Bearer ${token}` },
      });
      setBookingsData(
  ...
  const handleRegisterSchedule = () => {
    ...
    axios
      .post(
        'https://13.209.19.146:3000/api/trainer/schedule',
        {
          date: dateOnly,
  ...
  const handleDeleteSchedule = () => {
    ...
    axios
      .delete(`https://13.209.19.146:3000/api/trainer/schedule/${deleteTargetId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then(() => {
        alert('스케줄이 삭제되었습니다.');
