...
| (5) Summary |  
1. 운동 기록 및 주간 리포트 생성<br>  
- React Native 기반 동적 폼으로 운동 기록 입력 및 오운완 사진 촬영·업로드 가능 (AWS S3 연동)<br>  
- 주간/월간 리포트를 시각화하여 운동 패턴 분석 지원<br>  
- 트레이너는 회원별 운동 기록 열람 및 피드백 기능 제공<br>  
- 백엔드는 운동 기록을 정형 데이터로 관리하고, 역할 기반 JWT 인증/권한 검증 구현<br><br>  

2. 회원가입 및 트레이너-회원 연결<br>  
- 로그인·회원가입 통합 화면과 역할 기반 분기 네비게이션 구현<br>  
- 트레이너는 회원의 고유 코드를 입력해 직접 연결 가능, 회원은 단일 트레이너만 연결 허용<br>  
- 백엔드는 bcrypt 기반 해싱, 역할별 API 접근 제어 및 TrainerMembers 테이블로 관계 관리<br><br>  

3. 운동 스트릭 기능 (Workout Streak)<br>  
- 캘린더 UI로 운동 연속성 시각화 및 루틴 관리 지원<br>  
- 주 단위 운동 기록 집계를 통한 습관 형성 유도<br>  
- 백엔드는 단일 API로 주간 운동 연속 기록 여부 반환<br><br>  

4. 수강 신청 및 트레이너 스케줄 예약<br>  
- 트레이너는 드래그 UI로 수업 시간 설정, 회원은 예약 및 중복 방지된 스케줄 확인 가능<br>  
- 예약 24시간 전까지만 취소 허용, 실시간 UI 동기화<br>  
- 백엔드는 TrainerSchedule/MemberBookings로 스케줄 및 예약 정보 관리<br><br>  

5. 실시간 운동 알림 기능 (Electron 연동)<br>  
- Electron 앱과 연동된 실시간 알림 설정 및 스케줄 기반 자동 등록<br>  
- node-schedule과 cron 형식으로 알림 예약, OS/앱 내부 알림 병행<br>  
- 로그인 상태 및 스케줄 변경 시 자동 재등록 및 충돌 방지 |
|
<br>